...
# Convert text data into TF-IDF features and train the GradientBoostingClassifier using a pipeline
pipeline = make_pipeline(
    TfidfVectorizer(max_features=5000),  # You can adjust the number of features
    GradientBoostingClassifier(n_estimators=100, learning_rate=0.1, max_depth=3, random_state=42)  # Tunable parameters
)
...
with open('gbm_model_cv.pkl', 'wb') as model_file:
...
with open('gbm_model_70_30.pkl', 'wb') as model_file_70_30:
...
with open('gbm_model_80_20.pkl', 'wb') as model_file_80_20:
